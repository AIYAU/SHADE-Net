...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, tar_lsample_num_per_class, shot_num_per_class, HalfWidth):
    ...
    # Precompute indices by class label (1..m) for CutMix partner sampling
    m = int(np.max(G))
    indices_by_class = {}
    for i in range(m):
        indices_by_class[i+1] = [j for j, _ in enumerate(Row.ravel().tolist()) if G[Row[j], Column[j]] == i + 1]
    ...
    for iSample in range(da_nTrain):
        # base patch and label
        base_row = Row[da_RandPerm[iSample]]
        base_col = Column[da_RandPerm[iSample]]
        lab = G[base_row, base_col].astype(np.int64)  # label in 1..m
        base_patch = data[base_row - HalfWidth: base_row + HalfWidth + 1,
                          base_col - HalfWidth: base_col + HalfWidth + 1, :]
        # select partner from the same class
        cand = indices_by_class[lab]
        partner_choice = cand[np.random.randint(0, len(cand))]
        partner_row = Row[partner_choice]
        partner_col = Column[partner_choice]
        mix_patch = data[partner_row - HalfWidth: partner_row + HalfWidth + 1,
                         partner_col - HalfWidth: partner_col + HalfWidth + 1, :]
        # CutMix then Mask
        mixed_masked = data_augment.cutmix_then_mask_numpy(
            base_patch, mix_patch,
            mix_min=0.3, mix_max=0.7,
            mask_min=0.2, mask_max=0.5,
            mask_value=0.0,
            add_radiation_noise=False)
        imdb_da_train['data'][:, :, :, iSample] = mixed_masked
        imdb_da_train['Labels'][iSample] = lab
...
def get_HBKC_data_loader(task, num_per_class=1, split="train", shuffle=False):
    ...
    if split == "train":
        num_inst = task.support_num
    else:
        num_inst = task.query_num
    sampler = ClassBalancedSampler(num_per_class=num_per_class, num_cl=task.num_classes, num_inst=num_inst, shuffle=shuffle)
