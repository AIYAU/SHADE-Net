...
from model.encoder import Encoder, DynamicMarginAdaptation
...
for iDataSet in range(nDataSet):
    ...
    # 动态边界适应机制
    dmam = DynamicMarginAdaptation(feature_dim=emb_size, num_classes=TAR_CLASS_NUM).to(GPU)
    ...
    dmam_optim = torch.optim.SGD(dmam.parameters(), lr=LEARNING_RATE, momentum=0.9, weight_decay=WEIGHT_DECAY)

    mapping_src.apply(utils.weights_init)
    mapping_tar.apply(utils.weights_init)
    encoder.apply(utils.weights_init)
    dmam.apply(utils.weights_init)
    ...
    for episode in range(EPISODE):
        ...
        support_features_src, semantic_feature_src, support_spatial_src, support_spectral_src, decoded_src = encoder(mapping_src(support_src.to(GPU)),
                                                                                                        semantic_feature=semantic_support_src.to(GPU),
                                                                                                        s_or_q="support")  # (9, 160)
        query_features_src, query_spatial_src, query_spectral_src = encoder(mapping_src(query_src.to(GPU)))

        support_features_tar, semantic_feature_tar, support_spatial_tar, support_spectral_tar, decoded_tar = encoder(mapping_tar(support_tar.to(GPU)),
                                                                                                        semantic_feature=semantic_support_tar.to(GPU),
                                                                                                        s_or_q="support")  # (9, 160)
        query_features_tar, query_spatial_tar, query_spectral_tar = encoder(mapping_tar(query_tar.to(GPU)))

        # 应用动态边界适应机制
        support_features_src, margins_src = dmam(support_features_src, support_real_labels_src)
        support_features_tar, margins_tar = dmam(support_features_tar, support_real_labels_tar)
        ...
        # 解耦式光谱-空间对比学习
        # 计算光谱特征的对比损失
        spectral_align_loss = infoNCE_Loss(semantic_feature_src, support_spectral_src) + infoNCE_Loss(semantic_feature_tar, support_spectral_tar)
        # 计算空间特征的对比损失
        spatial_align_loss = infoNCE_Loss(semantic_feature_src, support_spatial_src) + infoNCE_Loss(semantic_feature_tar, support_spatial_tar)

        # 跨分支一致性约束
        consistency_loss = torch.mean(torch.abs(F.normalize(support_spectral_src, dim=1) - F.normalize(support_spatial_src, dim=1)))
        consistency_loss += torch.mean(torch.abs(F.normalize(query_spectral_src, dim=1) - F.normalize(query_spatial_src, dim=1)))
        consistency_loss += torch.mean(torch.abs(F.normalize(support_spectral_tar, dim=1) - F.normalize(support_spatial_tar, dim=1)))
        consistency_loss += torch.mean(torch.abs(F.normalize(query_spectral_tar, dim=1) - F.normalize(query_spatial_tar, dim=1)))
        consistency_loss *= 0.5

        # 自编码器损失
        reconstruction_loss = torch.mean(torch.abs(decoded_src - semantic_support_src.to(GPU)))
        reconstruction_loss += torch.mean(torch.abs(decoded_tar - semantic_support_tar.to(GPU)))
        reconstruction_loss *= 0.5

        # target domain supervised contrastive learning
        try:
            target_ssl_data, target_ssl_label = target_ssl_iter.__next__()
        except Exception as err:
            target_ssl_iter = iter(target_ssl_dataloader)
            target_ssl_data, target_ssl_label = target_ssl_iter.__next__()

        # Use rectangular region masking to create two SSL views
        if hasattr(data_augment, 'random_rect_mask_batch_image'):
            augment1_target_ssl_data = data_augment.random_rect_mask_batch_image(target_ssl_data.data.cpu(),
                                                                                 min_ratio=0.2, max_ratio=0.5,
                                                                                 mask_value=0.0)
            augment2_target_ssl_data = data_augment.random_rect_mask_batch_image(target_ssl_data.data.cpu(),
                                                                                 min_ratio=0.2, max_ratio=0.5,
                                                                                 mask_value=0.0)
        else:
            # Fallback to pixel-level random mask if rectangular mask is unavailable
            augment1_target_ssl_data = data_augment.random_mask_batch_image(target_ssl_data.data.cpu(), mask_ratio=0.4)
            augment2_target_ssl_data = data_augment.random_mask_batch_image(target_ssl_data.data.cpu(), mask_ratio=0.4)
        augment_target_ssl_data = torch.cat((augment1_target_ssl_data, augment2_target_ssl_data),
                                            dim=0)  # (2B, C, H, W)
        features_augment, _, _ = encoder(mapping_tar(augment_target_ssl_data.to(GPU)), use_attention=True)  # (2B, 128)

        augment1_target_ssl_feature = F.normalize(features_augment[:len(target_ssl_data), :], dim=1)  # (B, 128)
        augment2_target_ssl_feature = F.normalize(features_augment[len(target_ssl_data):, :], dim=1)  # (B, 128)
        ...
        loss = f_loss + 2.0 * text_align_loss + 1.0 * spectral_align_loss + 1.0 * spatial_align_loss + 0.5 * consistency_loss + 1.0 * reconstruction_loss + 2.0 * scl_loss_tar
        ...
        if (episode + 1) % 100 == 0:
            logger.info(
                'episode: {:>3d}, f_loss: {:6.4f}, text_align_loss: {:6.4f}, spectral_align_loss: {:6.4f}, spatial_align_loss: {:6.4f}, consistency_loss: {:6.4f}, reconstruction_loss: {:6.4f}, scl_loss_tar: {:6.4f}, loss: {:6.4f}, acc_src: {:6.4f}, acc_tar: {:6.4f}'.format(
                    episode + 1,
                    f_loss.item(),
                    text_align_loss.item(),
                    spectral_align_loss.item(),
                    spatial_align_loss.item(),
            ...
            writer.add_scalar('Loss/spectral_align_loss', spectral_align_loss.item(), episode + 1)
            writer.add_scalar('Loss/spatial_align_loss', spatial_align_loss.item(), episode + 1)
            writer.add_scalar('Loss/consistency_loss', consistency_loss.item(), episode + 1)
            writer.add_scalar('Loss/reconstruction_loss', reconstruction_loss.item(), episode + 1)
        ...
        if (episode + 1) % 500 == 0 or episode == 0:
            with torch.no_grad():
                ...
                train_features, _, _, _, _ = encoder(mapping_tar(Variable(train_datas).to(GPU)),
                                                semantic_feature=semantic_support.to(GPU), s_or_q="support")
                ...
                for test_datas, test_labels in test_loader:
                    ...
                    test_features, _, _ = encoder(mapping_tar((Variable(test_datas).to(GPU))))
